...
import '../kiosk/PatientDashboard.css';
...
const PatientDashboard = () => {
    ...
    const [aiInsight, setAiInsight] = useState("AI Tip: Balanced nutrition is the foundation of long-term wellness.");
    
    // Modal states
    const [isEditModalOpen, setIsEditModalOpen] = useState(false);
    const [editData, setEditData] = useState({
        name: '', phone: '', email: '', dob: '', address: '', blood_type: '', emergency_contact_name: '', emergency_contact_phone: ''
    });
    ...
    const handleSubmitReview = async () => {
        ...
        try {
            ...
            fetchData();
            showToast("Feedback Submitted", "Thank you for sharing your experience.", "success");
        } catch (err) {
            console.error("Failed to submit review:", err);
            showToast("Review Failed", "You may have already submitted feedback for this session.", "error");
        } finally {
    ...
    return (
        <div className="w-screen h-screen overflow-hidden flex font-body bg-slate-50 text-on-surface dashboard-container">
            <PatientSidebar activeItem="dashboard" patient={patient} />

            <main className="flex-1 flex flex-col relative overflow-hidden bg-white">
                <KioskTopBar title="Personal Dashboard" patientName={patient?.full_name} showHelp={false} />
                            ...
                            <div className="col-span-8 glass-card rounded-[2.5rem] p-8 shadow-lg relative overflow-hidden border border-white">
                                ...
                                <div className="relative z-10 flex items-start gap-8">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="grid grid-cols-4 gap-6 mt-8 text-left">
                                        ...
                                        {isMinor && (
                                            <div className="mt-8 p-6 bg-primary/5 rounded-[2rem] border border-primary/10 flex items-center gap-8 animate-in fade-in slide-in-from-top-2 duration-500">
                                                ...
                                                <div className="grid grid-cols-3 flex-1 gap-6 text-left">
                            ...
                            <div className="col-span-4 space-y-4">
                                <div 
                                    ...
                                    className={`rounded-[2rem] p-6 shadow-xl transition-all cursor-pointer group ${
                                        (queue && queue.status) 
                                            ? 'bg-gradient-to-br from-primary to-blue-700 text-white shadow-primary/20 hover:scale-[1.01]' 
                                            : 'bg-slate-50 border-2 border-dashed border-slate-200 text-slate-500 hover:scale-[1.01]'
                                    }`}
                                >
                                    ...
                                    <p className={`text-[11px] font-black uppercase tracking-widest mb-1 text-left ${ (queue && queue.status) ? 'text-white/80' : 'text-slate-400'}`}>
                                        {(queue && queue.status) ? queue.status : 'NO ACTIVE SESSION'}
                                    </p>
                                    <h3 className="text-2xl font-black font-headline leading-tight text-left">
                                        {(queue && queue.status) ? (
                                    ...
                                    <div className="mt-4 flex items-center gap-3 text-left">
                                        {(queue && queue.status) ? (
                                            ...
                                            <p className="text-[10px] font-bold text-slate-400">Click to track real-time clinic queues</p>
                                        )}
                                    </div>
                                </div>
                                ...
                                <div 
                                    ...
                                    className="glass-card rounded-[2rem] p-6 shadow-md hover:shadow-lg hover:scale-[1.01] transition-all cursor-pointer group border-primary/20 bg-primary/5 text-left"
                                >
                                    ...
                                    {history.length > 0 ? history.map((appt, idx) => (
                                        ...
                                        >
                                            <div className="flex items-center gap-4 text-left">
                                                <div className="w-12 h-12 rounded-xl bg-white shadow-sm flex flex-col items-center justify-center border border-slate-100 shrink-0">
                    ...
                    <div className="w-full max-w-md bg-white rounded-[2.5rem] p-8 shadow-2xl border border-white animate-in zoom-in-95 duration-300">
                        ...
                        <div className="space-y-5">
                            ...
                            <div className="border-t border-slate-100 pt-4 mt-2">
                                <p className="text-[10px] font-black text-red-600 uppercase tracking-widest px-1 mb-3 flex items-center gap-1.5 text-left font-bold">
                ...
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4 no-print">
                    <div className="bg-white rounded-[2.5rem] w-full max-w-lg overflow-hidden shadow-2xl animate-in zoom-in duration-300">
                        ...
                        <div className="p-8 space-y-6 text-left">
                            ...
                            <div className="space-y-2 p-6 bg-slate-50 rounded-3xl border border-slate-100 text-left">
                            ...
                            <div className="flex gap-4">
                                <button 
                                    ...
                                    className="flex-1 py-4 bg-slate-100 rounded-2xl font-bold text-on-surface-variant hover:bg-slate-200 transition-all font-bold text-sm"
                                >
                                    Close
                                </button>
                                {!selectedHistoryItem.status?.toLowerCase()?.includes('cancel') && selectedHistoryItem.status?.toLowerCase() !== 'completed' && (
                                    <>
                                        <button 
                                            ...
                                            className="flex-1 py-4 bg-primary text-white rounded-2xl font-bold shadow-lg shadow-primary/20 hover:scale-[1.02] transition-all flex items-center justify-center gap-1.5 font-bold text-sm"
                                        >
                                        ...
                                        <button 
                                            ...
                                            className="flex-1 py-4 bg-red-500 text-white rounded-2xl font-bold shadow-lg shadow-red-500/20 hover:scale-[1.02] hover:bg-red-600 transition-all flex items-center justify-center gap-1.5 font-bold text-sm"
                                        >
                    ...
                    <div className="bg-white rounded-[2.5rem] w-full max-w-lg overflow-hidden shadow-2xl animate-in zoom-in duration-300">
                        ...
                        <div className="p-8 space-y-6 text-left">
                            ...
                            <div className="flex gap-4 pt-4">
                                <button type="button" onClick={() => setShowRescheduleModal(false)} className="flex-1 py-4 rounded-2xl font-bold text-outline hover:bg-slate-100 transition-all font-bold text-sm">Cancel</button>
                                <button 
                                    onClick={submitReschedule}
                                    disabled={!selectedNewSessionId}
                                    className={`flex-[2] py-4 rounded-2xl font-bold transition-all shadow-lg font-bold text-sm ${
                ...
                <div className="fixed top-6 right-6 z-[9999] max-w-sm w-full bg-white/85 backdrop-blur-md rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.1)] border border-slate-100/50 overflow-hidden animate-in slide-in-from-top-10 slide-in-from-right-10 duration-300 no-print">
                    <div className="p-4 flex gap-3.5 items-start">
                        ...
                        <div className="flex-1 text-left min-w-0 font-headline">
                            <h4 className="text-sm font-extrabold text-slate-800 leading-tight mb-0.5 text-left">
                                {toast.title}
                            </h4>
        ...
        <div className="bg-white rounded-[3rem] w-full max-w-2xl shadow-2xl overflow-hidden border border-white flex flex-col max-h-[80vh]">
            ...
            <div className="p-10 overflow-y-auto space-y-4">
                {prescriptions.length === 0 ? (
                    ...
                    prescriptions.map(p => (
                        <div key={p.id} className="p-6 bg-slate-50 rounded-3xl border border-slate-100 text-left">
                            <div className="flex justify-between items-start mb-4">
                            ...
                            {p.instructions && (
                                <div className="mt-3 p-4 bg-white rounded-2xl border border-slate-200 border-l-4 border-l-primary">
                                    <p className="text-[10px] font-black text-primary uppercase tracking-widest mb-1 text-left">Pharmacist Instructions:</p>
                                    <p className="text-sm font-bold text-slate-600 text-left">{p.instructions}</p>
                                </div>
                            )}
                            {p.attachment && (
                                <div className="mt-4 pt-4 border-t border-slate-200">
                                    <button 
                                        ...
                                        className="w-full flex items-center justify-center gap-2 py-3 bg-white text-primary border border-primary/20 rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-primary/5 transition-all shadow-sm font-bold"
                                    >
        ...
        <div className="bg-white rounded-[3rem] w-full max-w-2xl shadow-2xl overflow-hidden border border-white flex flex-col max-h-[80vh]">
            ...
            <div className="p-10 overflow-y-auto space-y-4">
                {reports.length === 0 ? (
                    ...
                    reports.map(r => (
                        <div key={r.id} className="p-6 bg-slate-50 rounded-3xl border border-slate-100 text-left">
                            <div className="flex justify-between items-start mb-4">
                            ...
                            {r.result_summary && (
                                <div className="mt-3 p-4 bg-white rounded-2xl border border-slate-200 border-l-4 border-l-secondary text-left">
                            ...
                            {r.attachment && (
                                <div className="mt-4 pt-4 border-t border-slate-200">
                                    <button 
                                        ...
                                        className="w-full flex items-center justify-center gap-2 py-3 bg-white text-secondary border border-secondary/20 rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-secondary/5 transition-all shadow-sm font-bold"
                                    >
                                        <span className="material-symbols-outlined text-lg">visibility</span>
                                        View Attached Document
                                    </button>
